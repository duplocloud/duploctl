...
config file (default ``~/.duplo/config``). The ``DuploCtl`` controller
loads this resource as a service for its read path (``settings``/
``context``); the CLI dispatches the same class for the read/write
commands. Concurrent writers are last-writer-wins; the atomic replace
only guarantees readers never see a torn file.
"""
import copy
import os
import yaml
...
class DuploConfigResource(DuploResource):
  ...
  current context. The parsed document is cached after first read;
  ``save()`` refreshes the cache so readers sharing this instance never
  go stale — the controller's ``settings``/``context`` read through its
  ``config_svc`` handle on this same class. Not to be confused with
  ``duploctl tenant config``, which edits tenant metadata through the
  portal API.
  """
  def __init__(self, duplo):
    super().__init__(duplo)
    self.path = duplo.config_file
    self._data = None
  ...
  def get(self, key: args.CONFIG_KEY) -> dict:
    ...
    """
    ctx = self.current_context_name(self.duplo._context)
    return {"context": ctx, key: self.get_key(key, ctx)}
  ...
  def set(self, key: args.CONFIG_KEY, value: args.CONFIG_VALUE) -> dict:
    ...
    Raises:
      DuploError: If the key is invalid or no context is selected.
    """
    ctx = self.set_key(key, value, self.duplo._context)
  ...
  def unset(self, key: args.CONFIG_KEY) -> dict:
    ...
    Raises:
      DuploError: If the key is invalid or no context is selected.
    """
    ctx = self.unset_key(key, self.duplo._context)
