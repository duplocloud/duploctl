...
- `hd_cronjob update_image <name> <image>` — updates the first container image of an AI HelpDesk CronJob's job template, mirroring the core platform's `cronjob update_image`. Implemented as a fetch-patch-update of the record (the backend has no dedicated cronjob update-image endpoint); honors `--wait`.
