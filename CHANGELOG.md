...
- **Headless login** (`--headless` / `--no-browser`, `DUPLO_HEADLESS`) for machines that cannot open a browser, such as a remote host over ssh or a container. duploctl prints the login url, the user opens it in a browser anywhere, and pastes back the `http://localhost:56789/?t=...` address the browser was redirected to. The token is parsed from the url and cached like any other interactive login.
  - `--headless-port` (`DUPLO_HEADLESS_PORT`) receives the callback on a fixed, forwarded port (`ssh -L 56789:localhost:56789`) instead of prompting for a paste, so it also works where there is no terminal, e.g. an AWS `credential_process` or a kubectl exec credential plugin.
  - The callback listens on loopback by default; `--headless-bind` (`DUPLO_HEADLESS_BIND`) widens it for a container whose port is published.
  - Paste mode leaves a long lived token in browser history, so the prompt and the docs say so and point at `--headless-port` instead.
  - All three flags imply `--interactive`, are carried into the commands generated by `jit update_aws_config` and `jit update_kubeconfig`, and can be set in a config context.
  - The browser login now warns and prints the login url when no browser could be launched, instead of silently waiting for a callback that will never come, and fails fast with a pointer to `--headless` when `--browser` names a browser that is not installed.
  - Thanks to [@thegreatco](https://github.com/thegreatco) for the original contribution in [#278](https://github.com/duplocloud/duploctl/pull/278).
