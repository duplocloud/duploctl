...
- New `aws_credentials` resource — mints just-in-time AWS credentials from an AI HelpDesk (the helpdesk counterpart to `jit aws`). `list` shows the workspace's AWS cloud scopes; `find` mints STS credentials for a scope by name/id (defaulting to the workspace's only AWS scope) or, with `--rg`, for the IAM role attached to a resource group. Returns the access key, secret, session token, region, expiry, and a federated console URL.
...
- **`config` resource** — manage the local duploctl config file from the CLI: `config view` (tokens redacted), `config get`/`set`/`unset <key>` on the current context (or `--ctx <name>`, which also creates/scaffolds missing contexts and files), and `config use <name>` to switch `current-context`. Keys are validated against a strict allowlist that already includes the upcoming AI HelpDesk keys (`environment`, `resource_group`, `helpdesk_host`, `helpdesk_token`). Config-file loading moved out of `DuploCtl` into a shared `duplocloud.config.DuploConfig` store; writes are atomic and chmod the file to `0600`, but rewriting does strip YAML comments.
- `workspace use <name>` — kubectl-style sticky selection: validates the workspace against the portal, then persists its canonical name to the current config context (equivalent to `duploctl config set workspace <name>`).
